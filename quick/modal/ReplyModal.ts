...
//
//     if (user?.id) {
//         const replys: IReply[] = await getUserReply(read);
//
//         console.log(replys);
//         if (replys && replys.length > 0) {
//             for (let reply of replys) {
//                 block.addSectionBlock({
//                     text: { text: `${reply.name}`, type: TextObjectType.PLAINTEXT },
//                 });
//                 const newbody = reply.body.slice(0, 60);
//
//                 block.addContextBlock({ elements: [{ type: TextObjectType.PLAINTEXT, text: `${newbody}` }] })
...
//         } else {
//             block.addSectionBlock({
//                 text: { text: "You have no quick reply yet.", type: TextObjectType.PLAINTEXT }
//             });
//         }
//     }
//
